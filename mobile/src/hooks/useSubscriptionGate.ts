...
export interface SubscriptionGate {
  ...
  /**
   * Max level index (0=Emerging … 3=Master) the user can access.
  ...
  /** Returns true if level at levelIndex (0-3) is tier-locked regardless of unlock progress */
...
export function useSubscriptionGate(): SubscriptionGate {
  ...
  // max_level: 1=Emerging only (index 0), 2=+Developing (index 0-1), -1=all
  const maxLevelIndex: number =
    typeof features?.max_level === 'number'
      ? features.max_level === -1 ? 3 : features.max_level - 1
